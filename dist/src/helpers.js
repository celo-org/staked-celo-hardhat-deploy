            ...
            if (!proxy) {
                ...
                if (proxyAdminName) {
                    ...
                    if (changeImplementationMethod === 'upgradeToAndCall' &&
                        !updateMethod) {
                        executeReceipt = await execute(name, Object.assign(Object.assign({}, options), { from: proxyAdmin }), 'upgradeTo', implementation.address);
                    }
                    else {
                        executeReceipt = await execute(name, Object.assign(Object.assign({}, options), { from: proxyAdmin }), changeImplementationMethod, implementation.address, data);
                    }
                    if (!executeReceipt) {
                        throw new Error(`could not execute ${changeImplementationMethod}`);
                    }
